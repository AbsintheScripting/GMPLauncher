...
#define COMMON_DEFAULT_GRANULARITY 16
	...
	template <class type,
	          int granularity = COMMON_DEFAULT_GRANULARITY,
	          typename sizetype = uInt,
	          typename sorttype = int>
	class Array : public Object
	{
	public:
		using CompFunc = int(*)(const type& obj1, const type& obj2, Object* arg);
		using MoveFunc = void(*)(type* to, const type* from, sizetype size);
		using ZeroFunc = void(*)(type* first, sizetype count);
	...
	class ObjectArray final : public Array<type, granularity, sizetype>
	{
	public:
		...
		const ObjectArray<type, granularity, sizetype>& operator =(
			const ObjectArray<type, granularity, sizetype>& objects)
		{
			Clear();
			Add(objects);
			return *this;
		...
		ObjectArray(sizetype startsize = 0)
			: Array(startsize)
		{
			Copier = DefaultCopier;
			Mover = DefaultMover;
		...
		ObjectArray(const ObjectArray<type, granularity, sizetype>& objects)
			: Array(0)
		{
			Copier = DefaultCopier;
			Mover = DefaultMover;
