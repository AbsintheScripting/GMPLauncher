...
namespace COMMON
{
	...
	public:
		...
		public:
			Container(HashTable<type>* table, const char* name, const uInt length)
			{
				Array = table;
				Named = true;
			...
			{
				type data = static_cast<type>(Cont);
				(*this) = data;
				return data;
			}
	...
	private:
		...
		public:
			...
			void Set(const type& data, const uChar* name, const uInt len)
			{
				Data = data;
				Name.Add(name, len);
			}

			bool CheckName(const uChar* name, const uInt len) const
			{
	...
	public:
		...
		{
			Container Cont((HashTable<type> *)this, (const char*)name, wcslen(name) * sizeof(wchar_t));
			return Cont;
		}

		Container operator [](const char* name) const
		{
			Container Cont((HashTable<type> *)this, name, (strlen(name)));
			return Cont;
		}

		Container operator [](const int num) const
		{
			Container Cont((HashTable<type> *)this, static_cast<uInt>(num));
			return Cont;
		}
