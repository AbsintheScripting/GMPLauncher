...
namespace COMMON
{
	...
	class TaggedArray final : public Object
	{
	public:
		class NodeEntry final : public Object
		{
		public:
			tagtype Tag;
			type Value;
		...
		using CompFunc = int(*)(const NodeEntry& obj1, const NodeEntry& obj2, Object* arg);
	...
	public:
		...
		type& operator [](const tagtype& tag) const
		{
			for (uInt i = 0; i < SubArray.Size(); i++)
			{
				if (SubArray[i].Tag == tag)
		...
		const TaggedArray<type, tagtype, granularity>& operator =(
			const TaggedArray<type, tagtype, granularity>& objects)
		{
			Clear();
			AddArray(objects);
			return *this;
		...
		bool GetDeduplicate() const
		{
			return Deduplicate;
		}

		void SetDeduplicate(const bool deduplicate)
		{
		...
		TaggedArray(const bool deduplicate = false, uInt startsize = 0)
		{
			Deduplicate = deduplicate;
		...
		TaggedArray(const TaggedArray<type, tagtype, granularity>& objects, const bool deduplicate = false)
		{
			Deduplicate = deduplicate;
			SubArray.SetComparator(DefaultComparator);
			AddArray(objects);
