...
namespace COMMON
{
	...
	{
		DWORD Size = 0;
		if (!GetComputerName(nullptr, &Size) && (GetLastError() == ERROR_BUFFER_OVERFLOW) && Size)
		{
			name.Resize(Size - 1);
	...
	bool PlatformGetExePath(TString& name)
	{
		...
		for (uInt i = 1; GetLastError() == ERROR_INSUFFICIENT_BUFFER; i++)
		{
			...
			SetLastError(0);
			DWORD Size = GetModuleFileName(nullptr, (TCHAR*)name.GetData(), i * MAX_PATH);
			if (!Size)
				return false;
