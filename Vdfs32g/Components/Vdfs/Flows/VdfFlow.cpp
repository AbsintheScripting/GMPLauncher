...
bool VdfFlow::UpdateIndex(VdfsIndex* index)
{
	if (Archive)
	{
		...
		DWORD readed = 0;
		auto Entries = new VdfEntryInfo[Header.NumEntries];
		size_t EntriesSize = Header.NumEntries * sizeof(VdfEntryInfo);
		if (!ReadFile(Archive, Entries, EntriesSize, &readed, nullptr) || (readed != EntriesSize))
		{
			delete[] Entries;
			return false;
		}
...
uLong VdfFlow::Read(const uLong offset, void* buffer, const uLong size)
{
	if (CurrentFileInfo)
	{
		if (SetFilePointer(Archive, CurrentFileInfo->Offset + offset, nullptr, FILE_BEGIN) == INVALID_SET_FILE_POINTER)
			return 0;

		DWORD readed = 0;
		if (size && ReadFile(Archive, buffer, size, &readed, nullptr) && (readed == size))
			return size;
	}
	return 0;
}
...
bool VdfFlow::Init(const TCHAR* arcname)
{
	...
	HANDLE hFile = CreateFile(arcname,
	                          GENERIC_READ,
	                          FILE_SHARE_READ,
	                          nullptr,
	                          OPEN_EXISTING,
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		DWORD readed = 0;
		if (ReadFile(hFile, &Header, sizeof(VdfHeader), &readed, nullptr) && (readed == sizeof(VdfHeader)))
		{
			if (!memcmp(Header.Signature, VDF_SIGNATURE_G2, sizeof(Header.Signature)) && (Header.Version == 0x50) && (
				SetFilePointer(hFile, Header.RootCatOffset, nullptr, FILE_BEGIN) != INVALID_SET_FILE_POINTER))
				Version = 2;
			else if (!memcmp(Header.Signature, VDF_SIGNATURE_G1, sizeof(Header.Signature)) && (Header.Version == 0x50)
				&& (SetFilePointer(hFile, Header.RootCatOffset, nullptr, FILE_BEGIN) != INVALID_SET_FILE_POINTER))
				Version = 1;
			else
			{
