...
BOOL SaveWndPos(void)
{
	...
	if (GetWindowPlacement(g_AppWindow, &Placement))
		if (RegCreateKeyEx(REG_ROOT_KEY,
		                   REG_ROOT_SUB,
		                   0,
		                   NULL,
		                   0,
...
BOOL LoadWndPos(void)
{
	...
	Placement.length = sizeof(WINDOWPLACEMENT);
	if (GetWindowPlacement(g_AppWindow, &Placement))
		if (ERROR_SUCCESS == RegOpenKeyEx(REG_ROOT_KEY,
		                                  REG_ROOT_SUB,
		                                  0,
		                                  KEY_QUERY_VALUE,
		                                  &Key))
...
BOOL AddModFile(LPCTSTR FilePath)
{
	...
	GetPrivateProfileString(INI_SEC_FILE,
	                        INI_FILE_VDF,
	                        TXT_INVALID,
	                        Buff,
	                        MAX_INFO,
	...
	while (*pSrc != TNUL)
	{
		if ((TSPC == *pStrVolume) && (TSPC == pStrVolume[1]))
		{
			*pStrVolume = TNUL;
			lstrcpy(pDst, pSrc);
			pDst += lstrlen(pSrc) + 1;
	...
	/* Desc */
	*Buff = TNUL;
	GetPrivateProfileString(INI_SEC_INFO,
	                        INI_INFO_DSC,
	                        SNUL,
	                        Buff,
	                        MAX_INFO,
	...
	*Buff = TNUL;
	...
	{
		int Idx = 0;
		LPTSTR pStrIcon = lstrend(Buff);
		while ((pStrIcon > Buff) && (*pStrIcon >= TEXT('0')) && (*pStrIcon <= TEXT('9')))
			pStrIcon--;
		/* Internal Imglist (G1) */
		if (pStrIcon == Buff)
		{
		...
		{
			/* Extract Icon (G2) */
			if ((pStrIcon > Buff) && (*pStrIcon == TEXT('-')))
				pStrIcon--;
			if ((pStrIcon > Buff) && (*pStrIcon == TEXT(',')))
			{
				*pStrIcon = TNUL;
	...
	{	/* Convert RTF \'xx into chars */
		...
		for (Dst = Src; Dst[-1] != TNUL; Dst++)
			if (Src[0] == TEXT('\\') &&
				Src[1] == TEXT('\'') &&
				((Src[2] >= TEXT('0') && Src[2] <= TEXT('9')) || (Src[2] >= TEXT('A') && Src[2] <= TEXT('F')) || (Src[2]
					>= TEXT('a') && Src[2] <= TEXT('f'))) &&
				((Src[3] >= TEXT('0') && Src[3] <= TEXT('9')) || (Src[3] >= TEXT('A') && Src[3] <= TEXT('F')) || (Src[3]
					>= TEXT('a') && Src[3] <= TEXT('f'))))
			{
...
void StartSelectedMod(HWND hDlg)
{
	...
	if (InterlockedExchange(&g_ModRunning, TRUE) == FALSE)
	{
		...
		DrawMenuBar(hDlg);
		{
			...
			while (pCurrHack)
			{
				if (!GetPrivateProfileString(pCurrHack->sec,
				                             pCurrHack->key,
				                             TXT_INVALID,
				                             pCurrHack->org,
				                             MAX_BUFF,
			...
			if (pos > 0)
			{
				...
				switch (pos)
				{
				case 1: lstrcat(cmnd, TEXT("4"));
					break;
				case 3: lstrcat(cmnd, TEXT("8"));
					break;
				case 4: lstrcat(cmnd, TEXT("10"));
					break;
				default: lstrcat(cmnd, TEXT("5"));
					break;
				}
				lstrcat(cmnd, TEXT(",s"));
				...
				do
				{
					...
					if (SmartHeap)
					{
						...
						if (SmartPID == pi.dwProcessId)
						{
							...
							SendMessage(SmartHeap,
							            WM_COMMAND,
							            BN_CLICKED << 16 | GetDlgCtrlID(OK_Button),
							            (LPARAM)OK_Button);
						}
					}
				}
...
INT_PTR CALLBACK DlgProc(HWND Dialog, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		g_ImageListGothic1 = ImageList_LoadBitmap(g_AppInstance,
		                                          MAKEINTRESOURCE(IDB_LIST),
		                                          32,
		                                          MAX_GOTHIC1_IMAGE_LIST,
		                                          RGB(0xFF, 0x00, 0xFF));
	...
	case WM_SIZE:
		...
		if ((SIZE_RESTORED == wParam) || (SIZE_MAXIMIZED == wParam))
		{
			...
			MoveWindow(g_RtfInfo,
			           rect.right,
			           dlgspace.y + playsize.y + itmspace.y,
			           playx,
			           rect.bottom - dlgspace.y - playsize.y - 2 * itmspace.y + 1,
			           TRUE);
			MoveWindow(g_RtfDesc,
			           dlgspace.x,
			           rect.bottom + 1,
			           LOWORD(lParam) - 2 * dlgspace.x + 1,
			           HIWORD(lParam) - rect.bottom - dlgspace.y - 1,
			           TRUE);
		}
		break;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!LoadString(hInstance, IDS_ANSI, GUI_RTF_ANSI, MAXSTR))
		lstrcpyn(GUI_RTF_ANSI,
		         TEXT(
			         "\\ansi\\ansicpg1252\\deflang1033\\deff0{\\fonttbl{\\f0\\fcharset0 MS Shell Dlg;}}\\fs16\\pard\\qc"),
		         MAXSTR);
	...
	if (g_AppWindow)
	{
		g_HookHandle = SetWindowsHookEx(WH_KEYBOARD, KeyboardProc, g_AppInstance, GetCurrentThreadId());
